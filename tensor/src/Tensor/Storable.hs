...
fromTensorList ::
  Tensor [] ->
  Tensor Vector
fromTensorList (Tensor dims elt es) =
  withStorableElt elt $
...
toTensorList ::
  Tensor Vector ->
  Tensor []
toTensorList (Tensor dims elt es) =
  withStorableElt elt $
...
normalize (Tensor dims e xs) =
  withStorableElt e $
    withOrdElt e $
      maybeFloatingElt e Nothing $
        Just $
